...
export const mongoController: mongoController = {
  ...
  getValueRecords: (req: Request, res: Response, next: NextFunction): void => {
    ...
    KeyData.find(
      { key: receivedKey, expired: true },
      function (err: NodeJS.ErrnoException, result: Buffer) {
        if (err) {
          const defaultErr = {
  ...
  createKVP: (req: Request, res: Response, next: NextFunction): void => {
    ...
    if (!res.locals.exists) {
      ...
      const existingKey = KeyData.findOne(
        { key: key, value: value, expired: false },
        function (err: NodeJS.ErrnoException, result: resultObject) {
          if (err) {
            const defaultErr = {
              log: 'ERROR found in mongoController.getKey',
  ...
  updateKeyName: (req: Request, res: Response, next: NextFunction): void => {
    ...
    KeyData.findOneAndUpdate(
      ...
      function (err: NodeJS.ErrnoException, result: Buffer) {
        ...
        KeyData.findOneAndUpdate(
          { key: newKey },
          { $push: { oldKeyNames: oldKeyName }, expired: false },
          function (err: NodeJS.ErrnoException, result: resultObject) {
            if (err) {
              const defaultErr = {
                log: 'ERROR found in mongoController.updateKeyName',
  ...
  updateValue: (req: Request, res: Response, next: NextFunction): void => {
    ...
    KeyData.findOne(
      ...
      function (err: NodeJS.ErrnoException, result: resultObject) {
        ...
        KeyData.findOneAndUpdate(
          { key: key, expired: false },
          { value: newValue, $push: { oldValues: oldValue } },
          function (err: NodeJS.ErrnoException, result: Buffer) {
            if (err) {
              const defaultErr = {
                log: 'ERROR found in mongoController.updateValue',
  ...
  deleteKey: (req: Request, res: Response, next: NextFunction): void => {
    ...
    KeyData.findOneAndUpdate(
      { key: key, expired: false },
      { expired: true, manualDelete: true },
      function (err: NodeJS.ErrnoException, result: resultObject) {
        if (err) {
          const defaultErr = {
  ...
  getKeyspaceMisses: (
    ...
    KeyData.find(
      ...
      function (err: NodeJS.ErrnoException, result: Buffer) {
        ...
        res.locals.keyspaceMisses = result;
        return next();
      }
    );
  }
