...
const client = new Redis(
  `redis://${USERNAME_REDIS}:${PASSWORD_REDIS}@${redisMockDB}`
  // host: HOST_REDIS,
  // port: PORT_REDIS,
...
export const redisController: redisController = {
  ...
  getLiveValue: async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      await client.get(key).then((result: Buffer) => {
        if (result) {
  ...
  appendValue: (req: Request, res: Response, next: NextFunction): void => {
    client
      .append(
        'req.body.keyName',
        'req.body.appendValue',
        function (err: NodeJS.ErrnoException, reply: Buffer) {
  ...
  deleteKey: async (req: Request, res: Response, next: NextFunction) => {
    ...
    console.log('hiya');
    try {
      await client.del(key).then((result: Buffer) => {
        if (result) {
